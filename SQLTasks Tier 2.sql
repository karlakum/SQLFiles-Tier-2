...
SELECT DISTINCT CONCAT (m.firstname, ' ', f.name) as tennis_bookings
FROM Members as m
...
SELECT f.name as facility, CONCAT(firstname,' ', surname) AS name,
 CASE
    WHEN m.memid = 0  THEN  guestcost * slots 
    ELSE membercost * slots
END AS total_cost,  DATE(starttime) as booking_day
FROM Members as m
INNER JOIN Bookings as b
ON m.memid = b.memid
INNER JOIN Facilities as f
...
INNER JOIN Facilities as f
ON b.facid = f.facid
WHERE DATE(starttime) in (SELECT DATE(starttime) FROM Bookings WHERE DATE(starttime) = '2012-09-14')
HAVING total_cost > 30
ORDER BY total_cost DESC;
